...
| 最低宿主 versionCode | `AiAgentPlugin.REQUIRED_HOST_VERSION` (当前 5283 为临时值, 路线图 P1.6 以交付契约模块与代理的宿主构建号回填) |
...
- 决策只接受 `AgentDecision` 扁平 JSON (附录 D); `DecisionValidator` 校验工具名与参数 Schema, 非法决策作为观察回送, 修复重试次数按 P0.2 决策点 (1 或 2) 固定, MUST NOT 无限重试.
