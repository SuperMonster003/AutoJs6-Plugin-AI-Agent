...
目前安裝版仍只顯示宿主狀態. P1 宿主介面及 P2.1-P2.4 工具, 決策, 執行器, 上下文和模型用戶端核心已實作並通過測試. 實際任務執行還需 P2.5 Binder/前台服務接入和 P3/P4 執行介接器. 腳本 API 與任務台分別在 P5/P6 落地. [ROADMAP.md](https://github.com/SuperMonster003/AutoJs6-Plugin-AI-Agent/blob/master/ROADMAP.md).
...
Agent 串行執行器與任務佇列 (1 個活動任務 + 8 個排隊任務), 支援模型/工具取消, 使用者互動逾時, 唯一終態事件與宿主失聯阻塞; 真實宿主接入仍在後續階段. Agent 確定性上下文裝箱, 支援位元組上限, 最近完整步驟對, 中英文提示與節點優先保留; 本機模型使用 3000 token 輸入預算和精簡工具簽名. 宿主模型用戶端核心, 驗證事件順序並支援 usage 記帳, 取消, 逾時和有限格式降級; 每次降級計入模型呼叫且保留決策修復額度.
...
> 宿主側邊欄已提供連線與管理入口; 目前插件尚不能執行任務. `ai.agent` API 與任務台仍分別屬於 P5 和 P6.
...
- `提示` 目前安裝版仍只顯示宿主狀態. P1 宿主介面及 P2.1-P2.4 工具, 決策, 執行器, 上下文和模型用戶端核心已實作並通過測試. 實際任務執行還需 P2.5 Binder/前台服務接入和 P3/P4 執行介接器. 腳本 API 與任務台分別在 P5/P6 落地.
- `提示` 宿主的 AI Agent 契約, 能力與模型代理, 畫面觀察, 腳本登記執行, 側邊欄與插件中心入口已實作; 插件任務執行仍在開發中
...
- `新增` Agent 串行執行器與任務佇列 (1 個活動任務 + 8 個排隊任務), 支援模型/工具取消, 使用者互動逾時, 唯一終態事件與宿主失聯阻塞; 真實宿主接入仍在後續階段
