...
1.0.0 提供自然語言任務, 已登記指令碼呼叫與分級確認的裝置操作. 已通過案例, 模型限制與待補裝置驗收見 [ROADMAP.md](https://github.com/SuperMonster003/AutoJs6-Plugin-AI-Agent/blob/master/ROADMAP.md). 原生工具呼叫, 視覺輸入與動態指令碼產生計劃於 1.1.0 支援.
...
開啟 AI Agent 並連接 AutoJs6, 輸入目標, 選擇預設後開始. 在任務卡片中回答詢問或確認操作, 點擊最近任務查看詳情.
...
參數類型支援 string, number, integer 和 boolean, 不支援巢狀物件或陣列. sensitive 指令碼執行前始終需要確認. 請只登記已審閱的指令碼, 風險聲明不會為 JavaScript 建立沙箱. [完整登記格式](https://github.com/SuperMonster003/AutoJs6/blob/master/docs/dev/agent-script-manifest-v1.md).
...
從任務台開啟 "預設" 儲存任務設定. 名稱是腳本與記憶的固定識別碼, 改名請複製預設. 內置 default 可編輯但不能刪除. 模型從宿主清單選擇, 也可保留自動選擇; 指定模型失效時失敗, 不自動換目標. 任務選項只能進一步收緊預設限制. 固定上下文與任務上下文合計最多 8 KiB. 記憶範圍可選全域及目前預設, 僅其中一種或關閉. 編輯或刪除預設不改變已排入佇列的任務. 私有儲存最多 32 個預設 / 1 MiB.
...
付款是獨立的敏感操作. 批准下單, 指令碼或同類操作不等於批准付款. 每次識別到的付款動作均需單獨確認, 逾時視為拒絕. 批准前請核對商戶, 商品, 地址及金額.
...
- `新增` 透過文字節點及可選的已授權 OCR 觀察畫面, 按節點參照點擊, 輸入, 捲動及按鍵, 並檢查畫面變化與完成證據
...
- `修復` 無安全鎖裝置喚醒時螢幕狀態尚未穩定, 導致任務浮動球無法恢復顯示的問題
- `修復` 外掛進程中斷的任務在重新啟動後正確記錄為失敗; 鎖屏觀察阻止後續螢幕操作
