...
Проверка хоста при подключении, очередь задач, ответы, отмена, запросы и закрытая история шагов; задачи блокируются при потере хоста и не возобновляются после перезапуска процесса. Задачи отправляет хост. Отдельная панель и API ai.agent по-прежнему запланированы на P5/P6.
...
- `Функция` Каталог зарегистрированных скриптов с кешем на 60 секунд для каждого подключения к хосту, детерминированным отбором до 24 кандидатов и ограниченными сводками параметров
...
- `Зависимость` Добавлены common-plugin-api, host-capability-api и ai-agent-api из одной release-сборки AutoJs6 6.8.0 / 5285 (MPL 2.0), закреплены SHA-256
