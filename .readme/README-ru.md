...
Установленная версия пока показывает только состояние хоста. Интерфейсы P1 и ядра инструментов, решений, исполнения, контекста и клиента модели P2.1-P2.4 реализованы и проверены. Реальное выполнение требует интеграции Binder/службы переднего плана P2.5 и адаптеров P3/P4. API скриптов и интерфейс задач появятся в P5/P6. [ROADMAP.md](https://github.com/SuperMonster003/AutoJs6-Plugin-AI-Agent/blob/master/ROADMAP.md).
...
Последовательный исполнитель Agent и очередь задач (1 активная + 8 ожидающих), отменяемые вызовы модели/инструментов, сроки диалогов, единственное итоговое событие и блокировка при потере хоста; реальное подключение в следующих этапах. Детерминированная сборка контекста Agent с лимитом байтов, полными недавними парами шагов, подсказками на английском/китайском и приоритетом узлов; локальный бюджет 3000 токенов и компактные описания инструментов. Клиент модели хоста с проверкой порядка событий, учетом usage, отменой, сроками и ограниченным переходом формата; каждый повтор учитывается как вызов и сохраняет лимит исправления решений.
...
- `Подсказка` Установленная версия пока показывает только состояние хоста. Интерфейсы P1 и ядра инструментов, решений, исполнения, контекста и клиента модели P2.1-P2.4 реализованы и проверены. Реальное выполнение требует интеграции Binder/службы переднего плана P2.5 и адаптеров P3/P4. API скриптов и интерфейс задач появятся в P5/P6.
