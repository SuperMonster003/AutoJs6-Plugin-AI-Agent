...
ホスト認証付き接続でタスクの待機列, 応答, 取消, 照会と非公開の手順履歴を提供; ホスト切断時は停止状態になり, プロセス再起動で自動再開しない. タスクはホストから送信します. タスク画面と ai.agent スクリプト API は P5/P6 の予定です.
...
- `機能` 登録済みスクリプトカタログのコアに, ホスト接続ごとの 60 秒キャッシュ, 最大 24 候補の決定的な順位付けとサイズ制限付きパラメーター要約を実装
