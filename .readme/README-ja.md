...
インストール版は現在もホスト状態のみを表示します. P1 ホストインターフェースと P2.1-P2.4 のツール, 決定, ランナー, コンテキスト, モデルクライアントのコアは実装とテストが完了しています. 実際の実行には P2.5 の Binder/フォアグラウンドサービス接続と P3/P4 の実行アダプターが必要です. スクリプト API とタスク画面は P5/P6 で実装します. [ROADMAP.md](https://github.com/SuperMonster003/AutoJs6-Plugin-AI-Agent/blob/master/ROADMAP.md).
...
Agent の直列ランナーとタスクキュー (実行中 1 件 + 待機 8 件), モデル/ツールのキャンセル, 対話期限, 一意の終了イベントとホスト切断時の停止を実装; 実際のホスト接続は後続段階. Agent の決定的なコンテキスト構築にバイト制限, 最近の完全なステップ対, 英語/中国語プロンプト, ノードの優先選択を実装; ローカル入力は 3000 token と簡潔なツール定義を使用. ホストモデルクライアントにイベント順序検証, usage 集計, キャンセル, 期限と有限の形式フォールバックを実装; 各再試行をモデル呼び出しとして計上し決定修正の上限を維持.
...
- `ヒント` インストール版は現在もホスト状態のみを表示します. P1 ホストインターフェースと P2.1-P2.4 のツール, 決定, ランナー, コンテキスト, モデルクライアントのコアは実装とテストが完了しています. 実際の実行には P2.5 の Binder/フォアグラウンドサービス接続と P3/P4 の実行アダプターが必要です. スクリプト API とタスク画面は P5/P6 で実装します.
...
- `機能` Agent の直列ランナーとタスクキュー (実行中 1 件 + 待機 8 件), モデル/ツールのキャンセル, 対話期限, 一意の終了イベントとホスト切断時の停止を実装; 実際のホスト接続は後続段階
