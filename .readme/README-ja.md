...
開発プレビュー: タスク画面, 履歴, プリセット, メモリ, 設定, リリース履歴と手動更新確認を利用できます. ai.agent API は AutoJs6 ビルド 5293 以降が必要です. フローティング表示と外部入口は P6.7, 信頼性と公開判定は P7/P8 で継続します. [ROADMAP.md](https://github.com/SuperMonster003/AutoJs6-Plugin-AI-Agent/blob/master/ROADMAP.md).
...
8. 前面ではタスク画面で回答し, 背景では高優先度の通知から該当するリクエストを開きます. 確認画面にツール, 引数, リスクと残り時間を表示します. 継続許可はこのタスクの同じツールと同じリスクのみで, 支払いと記憶の提案は毎回確認します. 回答の記憶は許可されたスコープ内で別の memory_propose を作成します. 通常の確認は 120 秒, 質問は最大 10 分で, タスクの予算も適用されます. 時間切れは USER_TIMEOUT として返り, モデルが再質問か部分完了を選びます. 古いリクエストでは新しい質問に回答できません. 背景の通知は権限とチャンネル設定に従います. フローティングカードは P6.7 で接続します.
...
> 開発プレビュー: タスク画面, 履歴, プリセット, メモリ, 設定, リリース履歴と手動更新確認を利用できます. ai.agent API は AutoJs6 ビルド 5293 以降が必要です. フローティング表示と外部入口は P6.7, 信頼性と公開判定は P7/P8 で継続します.
...
- INTERNET は GitHub の手動更新確認専用です. FOREGROUND_SERVICE と FOREGROUND_SERVICE_SPECIAL_USE は実行中タスク, POST_NOTIFICATIONS は進行状況と停止操作に使います. ユーザー補助, オーバーレイ, ストレージやマイクの権限は要求しません.
...
- `ヒント` 開発プレビュー: タスク画面, 履歴, プリセット, メモリ, 設定, リリース履歴と手動更新確認を利用できます. ai.agent API は AutoJs6 ビルド 5293 以降が必要です. フローティング表示と外部入口は P6.7, 信頼性と公開判定は P7/P8 で継続します.
